...
def remove_range(alist, min, max):
   ...
   *** Important: your code must use comprehensions and should not be more than
   two lines of code including the return statement ***
    """
    result = [i for i in alist if i < min or i > max]
    return result
...
def word_count(strings):
    ...
    *** Important: your code must use comprehensions and should not be more than two
    ...
    """
    result = {string:strings.count(string) for string in strings}
    return result
...
def friend_list(friend_dictionary):
    ...
    """
    result={}
    for person in friend_dictionary:
        if person not in result:
            result[person] = []
            if friend_dictionary[person] not in result:
                result[friend_dictionary[person]] = []    
        elif friend_dictionary[person] not in result:
            result[friend_dictionary[person]] = []
    
    for person in friend_dictionary:
        result[person].append(friend_dictionary[person])
        result[friend_dictionary[person]].append(person)
    return result
